...
st.write("Application interactive pour explorer le profil des 1000 clients.")
...
# (revenu, anciennete et score ne peuvent pas etre negatifs)
...
min_rev = float(df_clean["revenu_annuel"].min())
max_rev = float(df_clean["revenu_annuel"].max())
...
min_anc = float(df_clean["anciennete_client"].min())
max_anc = float(df_clean["anciennete_client"].max())
...
min_score = float(df_clean["score_fidelite"].min())
max_score = float(df_clean["score_fidelite"].max())
...
# PARTIE 2 : RELATIONS ENTRE VARIABLES
...
st.header("4. Quelles sont les anomalies ou valeurs extremes ?")
